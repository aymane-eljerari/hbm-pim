...
class PIMKernel {
public:
  PIMKernel(shared_ptr<MultiChannelMemorySystem> mem, int num_pim_chan,
            int num_pim_rank)
  ...
  void preprocessSrf(NumpyBurstType* input_npbst, fp16** params, int
  ...
  */
  ...
  void setControl(BurstType *bst, bool op, int crf_toggle_cond, bool grfA_zero,
                  bool grfB_zero);
  ...
  void preloadNoReplacement(NumpyBurstType *operand, unsigned startingRow,
                            unsigned startingCol);
  /*
  void preloadEltwise(NumpyBurstType* operand, pimBankType bank_types, unsigned
  ...
  */
  ...
  void executeKSKIP(int dim, pimBankType pb_type, KernelType ktype,
