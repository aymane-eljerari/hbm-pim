...
::testing::AssertionResult fp16EqualHelper(const char *m_expr,
                                           const char *n_expr, fp16 m, fp16 n);
::testing::AssertionResult fp16BstEqualHelper(const char *m_expr,
                                              const char *n_expr,
                                              DRAMSim::BurstType mb,
                                              DRAMSim::BurstType nb);
#define EXPECT_FP16_BST_EQ(val1, val2)                                         \
  EXPECT_PRED_FORMAT2(fp16BstEqualHelper, val1, val2)
#define EXPECT_FP16_EQ(val1, val2)                                             \
  EXPECT_PRED_FORMAT2(fp16EqualHelper, val1, val2)
...
public:
  TestStats() {}
  unsigned getNumPassed() { return num_test_passed_; }
  unsigned getNumWarning() { return num_test_warning_; }
  unsigned getNumFailed() { return num_test_failed_; }

  void IncNumPassed(unsigned i = 1) { num_test_passed_ += i; }
  void IncNumWarning(unsigned i = 1) { num_test_warning_ += i; }
  void IncNumFailed(unsigned i = 1) { num_test_failed_ += i; }
...
public:
  ...
  virtual void SetUp() { printTestMessage(); }
  ...
  BurstType *getResultPIM(KernelType kn_type, DataDim *dim_data,
                          shared_ptr<PIMKernel> kernel, BurstType *result) {
    switch (kn_type) {
    case KernelType::GEMV: {
      kernel->preloadGemv(&dim_data->weight_npbst_);
      kernel->executeGemv(&dim_data->weight_npbst_, &dim_data->input1_npbst_,
                          false);
      ...
      kernel->readResult(result, pimBankType::ODD_BANK,
                         dim_data->output_dim_ * dim_data->batch_size_, 0, 0,
                         end_col);
      break;
    }
    case KernelType::ADD:
    case KernelType::MUL: {
      ...
      int input_row1 = 128;
      int result_row = 256;
      kernel->preloadNoReplacement(&dim_data->input1_npbst_, input_row0, 0);
      kernel->preloadNoReplacement(&dim_data->input2_npbst_, input_row1, 0);
      kernel->executeEltwise(dim_data->dimTobShape(dim_data->output_dim_),
                             pimBankType::ALL_BANK, kn_type, input_row0,
  ...
  void expectAccuracy(KernelType kn_type, int num_tests,
                      NumpyBurstType precalculated_result,
                      uint32_t stride = 16) {
    switch (kn_type) {
    case KernelType::GEMV: {
      for (int i = 0; i < num_tests; i++) {
        ...
        reduced_result_[i / stride].fp16Data_[i % stride] =
            result_[i].fp16ReduceSum();
      }
      return;
    }
    case KernelType::ADD:
  ...
  shared_ptr<PIMKernel> make_pim_kernel() {
    shared_ptr<MultiChannelMemorySystem> mem =
        make_shared<MultiChannelMemorySystem>("ini/HBM2_samsung_2M_16B_x64.ini",
                                              "system_hbm_64ch.ini", ".",
                                              "example_app", 256 * 128 * 2);
    int numPIMChan = 64;
    int numPIMRank = 1;
    shared_ptr<PIMKernel> kernel =
        make_shared<PIMKernel>(mem, numPIMChan, numPIMRank);
  ...
  /* stats */
  void testStatsClear() { testStats.clear(); }

  void printTestMessage() { cout << ">>PIM Kernel Accuraccy Test" << endl; }

  void printFailVector() { testStats.printFailVector(); }
...
#define GET_NUM_TESTS()                                                        \
  testStats.getNumPassed() + testStats.getNumWarning() +                       \
      testStats.getNumFailed()
...
::testing::AssertionResult fp16EqualHelper(const char *m_expr,
  ...
  if (fp16Equal(m, n, 4, 0.01)) {
    ...
    return ::testing::AssertionFailure()
           << cur_idx << m_expr << " and " << n_expr << " (" << convertH2F(m)
           << " and " << convertH2F(n) << ") are not same " << mi.ival << " "
           << ni.ival;
  }
}

::testing::AssertionResult fp16BstEqualHelper(const char *m_expr,
                                              const char *n_expr, BurstType mb,
                                              BurstType nb) {
  for (int i = 0; i < 16; i++) {
